...
1. Preserve producer-defined fields such as `tags`, `resource`, `sources`, `verified`, `status`, `stale_after`, and Attested Computation fields.
1. Auto-generate `type`, `title`, `description`, and `generated` only when absent or as specified below.
1. Strip the input frontmatter from the body so it is not duplicated in output.
...
1. **Update frontmatter rendering**
...
1. **Update conversion entry points**
